...
function setState(raw){
  ...
  for(const [addr,p] of Object.entries(model.pendingVolumes)) {
    ...
    else if(p.acknowledged&&Date.now()>p.until) { delete model.pendingVolumes[addr];toast('Device volume has not confirmed the requested value.','error'); }
  }
...
async function api(path, options={}) {
  ...
  if (!res.ok) {
    let msg = 'The hub could not complete that request.';
...
function signal(rssi=0){const n=!rssi?0:rssi>=-60?4:rssi>=-70?3:rssi>=-80?2:rssi?1:0;return `<span class="signal">${[1,2,3,4].map(i=>`<i class="${i<=n?'on':''}"></i>`).join('')}</span>`}
...
function render(background=false) {
  ...
  try {
    ...
    app.innerHTML = `<div class="lock"><div class="lock-box">${logo()}${card(`<b>Interface recovered safely</b><p>The hub returned a state the interface could not display. Audio services were not changed.</p>${btn('Reload state','ui-retry','primary')} ${btn('Dashboard','ui-dashboard','secondary')}`)}</div></div>`;
  }
}
...
function renderShell() {
  ...
  patchHTML(app, `<div class="app-shell">
    <header class="topbar">${logo()}<nav>${routes.map(r=>`<button data-route="${r}" class="${r===model.route?'active':''}">${r}</button>`).join('')}</nav>
      <div class="top-status">${dot(model.realtime==='connected'?'connected':'warning')}<span>${esc(s.system.hostname)}</span></div>${btn('Sign out','logout','ghost')}</header>
...
function nodeCard(label,d,index,m,output=false){
  if(!d)return card(`<small>${label}</small><b>${output?'No output':index===1?'Add second source':'Add a source'}</b><span>${output?'Pair or assign a Bluetooth headset or speaker.':'Pair a phone or computer and assign it to this input.'}</span>${btn('Add device','go-devices')}`,'empty-slot node-card');
  ...
  return card(`<div class="device-title"><div><small>${label}</small><b title="${attr(d.name)}">${esc(d.name)}</b></div>${pill(state,connected?'Connected':d.status==='connecting'?'Connecting…':d.status==='error'?'Connection failed':'Disconnected')}</div>
    ...
    <div class="node-level"><span class="speaker-glyph">◖</span><input data-bt-volume="${attr(d.addr)}" type="range" min="0" max="100" step="1" value="${btVolume}" ${btControl?'':'disabled'}><output data-bt-volume-output="${attr(d.addr)}">${d.volumeKnown?`${btVolume}%`:'—'}</output></div>
...
function mergeConnector(a,b){
  const p1=a?'active':'idle', p2=b?'active':'idle';
  return `<div class="connector-svg merge"><svg viewBox="0 0 70 520" preserveAspectRatio="none" aria-hidden="true"><path class="${p1}" d="M0 130 C38 130 34 260 68 260"/><path class="${p2}" d="M0 390 C38 390 34 260 68 260"/><circle cx="68" cy="260" r="4"/></svg></div>`;
}
function outConnector(active){return `<div class="connector-svg out"><svg viewBox="0 0 70 520" preserveAspectRatio="none" aria-hidden="true"><path class="${active?'active':'idle'}" d="M2 260 C22 260 48 260 68 260"/><circle cx="2" cy="260" r="4"/></svg></div>`}
...
function dashboardPage(){
  ...
  return `<div class="page dashboard">${healthStrip()}${model.mixerError?`<div class="warning">${esc(model.mixerError)}${btn('Retry mixer save','mixer-retry')}</div>`:''}${s.health.wifi.warning?`<div class="warning"><span>2.4 GHz Wi‑Fi competes with Bluetooth audio. 5 GHz is recommended for multiple streams.</span>${btn('Network','go-network','ghost')}</div>`:''}
    ...
    <div class="signal-rail legacy-rail"><div class="input-column">${nodeCard('Input 1',d1,0,m)}${nodeCard('Input 2',d2,1,m)}</div>${mergeConnector(d1?.connected,d2?.connected)}
    ...
    ${outConnector(out?.connected)}<div class="output-column">${nodeCard('Output 1',out,0,m,true)}<div class="future-output">+ Add output <span>Later version</span></div></div></div></div>`;
}
...
function deviceCard(d){
  ...
  return card(`<div class="device-title"><div><small>${esc(kindLabel)}</small><b title="${attr(d.name)}">${esc(d.name)}</b></div>${pill(d.connected?'connected':'disconnected',d.connected?'Connected':d.paired?'Paired':'Available')}</div><div class="meta-row"><span>${esc(d.addr||'Unknown address')}</span><span>${signal(d.rssi)}${d.rssi||'—'}</span></div>
...
function devicesPage(){const s=model.state;let count=0;if(s.pairing.active&&s.pairing.until)count=Math.max(0,Math.ceil((new Date(s.pairing.until)-Date.now())/1000));return `<div class="page"><div class="page-title"><div><h1>Devices</h1><p>Pair Bluetooth devices, assign inputs and choose your output.</p></div></div>
  ...
  ${s.devices.length?`<div class="device-grid">${s.devices.map(deviceCard).join('')}</div>`:card('<b>No Bluetooth devices saved</b><p>Start pairing or scan to add an input or output.</p>','empty-slot')}</div>`}
...
function audioPage(){const s=model.state;if(!model.localAudio)model.localAudio=structuredClone(s.audio);const a=model.localAudio;const dirty=JSON.stringify(a)!==JSON.stringify(s.audio);return `<div class="page audio-page"><div class="page-title"><div><h1>Audio</h1><p>Change latency, sampling and stability settings. Applying restarts the audio graph.</p></div></div>
  ...
  ${card(`<h2>A/V synchronization · experimental</h2><label>Advertised total sink delay (ms)<input data-audio="secondaryAdvertisedDelayMs" type="number" min="0" max="2000" step="10" value="${a.secondaryAdvertisedDelayMs}"></label><p>0 leaves engine behavior unchanged. A nonzero value sends a fixed rendering-delay report from the secondary receiver. Enter a measured total delay, not an extra offset. This does not reduce physical latency, repair packet loss, or guarantee that a source app will synchronize its video. Primary input and multi-output synchronization are not implemented.</p>`)}</div>${dirty?`<div class="applybar"><span>Audio settings have unapplied changes.</span>${btn('Revert','audio-revert')}${btn('Apply & restart audio','audio-apply','primary')}</div>`:''}</div>`}
...
function systemPage(){const s=model.state.system;return `<div class="page"><div class="page-title"><div><h1>System</h1><p>Identity, software information and maintenance.</p></div></div><div class="settings-grid">${card(`<h2>Identity</h2><label>Hostname<input id="system-hostname" value="${attr(s.hostname)}"><small>${esc(s.hostname)}.local</small></label><label>Bluetooth name<input id="system-btname" value="${attr(s.btName)}"></label>${btn('Save identity','identity-save','primary')}`)}
  ...
  ${card(`<h2>Maintenance</h2><div class="stack-actions">${btn('Restart audio engine','system:restart-audio')}<a class="btn secondary" href="/api/system/backup">Download configuration backup</a>${btn('Reboot hub','system:reboot','danger')}</div>`)}
...
function diagnosticsPage(){const d=model.diagnostics||model.state.diagnostics;if(!d)return `<div class="page"><div class="page-title"><h1>Diagnostics</h1></div>${btn('Load diagnostics','diag-refresh')}</div>`;return `<div class="page"><div class="page-title"><div><h1>Diagnostics</h1><p>Live system health and transport details.</p></div><div>${btn('Refresh','diag-refresh')} ${btn('Copy diagnostics','diag-copy','primary')}</div></div><div class="diag-cards">${card('<small>CPU</small><b>'+d.cpuPercent.toFixed(1)+'%</b>')}${card('<small>Memory</small><b>'+d.memPercent.toFixed(1)+'%</b>')}${card(`<small>Temperature</small><b>${d.tempC?d.tempC.toFixed(1)+' °C':'—'}</b>`)}${card('<small>PipeWire errors</small><b>'+d.xruns+'</b>')}</div>
  ${section('Assigned audio paths',`<div class="table">${model.state.devices.filter(x=>x.role).map(x=>`<div class="tr"><span>${esc(roleLabel(x.role))}</span><span>${esc(x.name)}</span><span>${esc(x.backend||'not active')}</span><span>${x.connected?'connected':'disconnected'}</span></div>`).join('')}</div>`)}
  ${section('Bluetooth transports',`<div class="table">${d.transports.map(t=>`<div class="tr"><span>${esc(t.addr)}</span><span>${esc(t.codec||'—')}</span><span>${fmtRate(t.rate)}</span><span>${esc(t.state)}</span></div>`).join('')}</div>`)}
